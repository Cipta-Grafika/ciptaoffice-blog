    ...
    <div class="cms-page cms-page--narrow">
        ...
        <form class="cms-form-surface" method="post" action="{{ $user->exists ? route('cms.users.update', $user) : route('cms.users.store') }}">
                    ...
                    <div class="col-md-6 d-flex align-items-end pb-2">
                        <div class="w-100">
                            <label class="form-check mb-0">
                                ...
                                <input class="form-check-input @error('is_active') is-invalid @enderror" type="checkbox" name="is_active" value="1" @checked(old('is_active', $user->exists ? $user->is_active : true))>
            ...
            <section class="cms-form-section">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label">{{ $user->exists ? 'Kata sandi baru (opsional)' : 'Kata sandi' }}</label>
                        <input class="form-control @error('password') is-invalid @enderror" type="password" name="password" @required(!$user->exists)>
                        @error('password')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Konfirmasi</label>
                        <input class="form-control" type="password" name="password_confirmation" @required(!$user->exists)>
                    </div>
                </div>
            </section>
