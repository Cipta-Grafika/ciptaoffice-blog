        ...
        <table class="table align-middle mb-0">
            ...
            <tbody>
                @forelse($posts as $post)
                    <tr>
                        <td><strong>{{ $post->title }}</strong><small
                                class="d-block text-muted">/{{ $post->slug }}</small></td>
                        @can('admin')
                        ...
                        <td class="text-end">
                            ...
                            @else
                                @can('admin')
                                    <a class="btn btn-sm btn-outline-primary" href="{{ route('cms.posts.edit', $post) }}">Tinjau</a>
                                @endcan
                                @endcan @if ($post->status === \App\Enums\PostStatus::Published)
