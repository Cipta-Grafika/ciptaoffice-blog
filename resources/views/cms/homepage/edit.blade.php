    ...
    <div class="cms-page">
        ...
        <form class="cms-form-surface" method="post" action="{{ route('cms.homepage.update') }}" enctype="multipart/form-data">
            ...
            <section class="cms-form-section">
                ...
                <div class="row g-4">
                    <div class="col-md-6"><label class="form-label" for="eyebrow">Eyebrow</label><input
                            class="form-control" id="eyebrow" name="eyebrow"
                            value="{{ old('eyebrow', $settings->eyebrow) }}"></div>
                    <div class="col-12"><label class="form-label" for="title">Judul hero</label><input
                            class="form-control form-control-lg" id="title" name="title"
                            value="{{ old('title', $settings->title) }}" required></div>
                    <div class="col-12"><label class="form-label" for="summary">Ringkasan</label>
                        <textarea class="form-control" id="summary" name="summary" rows="4" required>{{ old('summary', $settings->summary) }}</textarea>
                    </div>
                </div>
            </section>
            <section class="cms-form-section">
                ...
                <div class="row g-4">
                    <div class="col-md-6"><label class="form-label" for="hero_image">Gambar hero</label><input
                            class="form-control" id="hero_image" type="file" name="hero_image"
                            accept="image/jpeg,image/png,image/webp"></div>
                    <div class="col-md-6"><label class="form-label" for="hero_image_alt">Alt text gambar</label><input
                            class="form-control" id="hero_image_alt" name="hero_image_alt"
                            value="{{ old('hero_image_alt', $settings->hero_image_alt) }}"></div>
                </div>
            ...
            <section class="cms-form-section">
                ...
                <div class="row g-4">
                    @foreach (['primary' => 'Utama', 'secondary' => 'Sekunder'] as $key => $label)
                        <div class="col-md-3"><label class="form-label" for="{{ $key }}_cta_label">Label CTA
                                {{ $label }}</label><input class="form-control" id="{{ $key }}_cta_label"
                                name="{{ $key }}_cta_label"
                                value="{{ old($key . '_cta_label', $settings->{$key . '_cta_label'}) }}"></div>
                        <div class="col-md-3"><label class="form-label" for="{{ $key }}_cta_url">URL CTA
                                {{ $label }}</label><input class="form-control" id="{{ $key }}_cta_url"
                                name="{{ $key }}_cta_url"
